...
func main() {
	...
	// Buat nama file dinamis dengan waktu
	dateStr := time.Now().Format("20060102_1504")
	baseFilename := fmt.Sprintf("backup_hse_mcu_jadwal_%s.sql", dateStr)
	...
	// Siapkan query
	quoted := make([]string, len(nipList))
	for i, nip := range nipList {
		quoted[i] = fmt.Sprintf("'%s'", nip)
	}
	inClause := strings.Join(quoted, ", ")
	query := fmt.Sprintf("SELECT * FROM hse_mcu_jadwal WHERE nip IN (%s)", inClause)
	...
	for rows.Next() {
		...
		insert := fmt.Sprintf("INSERT INTO hse_mcu_jadwal (%s) VALUES (%s);\n",
...
// Cek jika file sudah ada, tambahkan _backup1, _backup2, dst
